...
const SideBar = () => {
    ...
    <>
            ...
            <div className={`dark:bg-black dark:shadow-zinc-800 flex flex-col justify-between  transition-all w-1/3 h-full fixed right-0 top-0 z-50 pl-4 py-3 bg-white shadow-xl ${openSideBar}`}>
            <div className="w-full h-[12%] flex justify-between items-center">
              <div className="dark:bg-zinc-900 w-3/5 h-[90%] items-center flex pl-4 gap-4 overflow-hidden rounded-xl border cursor-pointer bg-yellow-50 hover:bg-yellow-100 transition-colors">
            ...
            <div className="w-full h-4/6 pl-4">
                <ul className="w-full h-full flex flex-col gap-14 justify-center">
                   {['Home','Posts','Categories','Contact Us'].map((item,index)=>(
                    <li key={index}><Link href={item} className="text-xl hover:pl-2 transition-all">{item}</Link></li>
                   ))} 
                </ul>
            </div>
            ...
            {fontSize === "medium" ? (
                <div onClick={()=>{setfontSize("large");document.documentElement.style.fontSize = "19px"}}><Text size="28" color={theme === "light" ? "#000" : "#fff"}/> Large</div>
              ) : (
                <div onClick={()=>{setfontSize("medium");document.documentElement.style.fontSize = "16px"}}><Text size="24" color={theme === "light" ? "#000" : "#fff"}/> Medium</div>
              )}
            </div>
