    ...
    return ( 
        <div className="w-full flex flex-col items-center">
            <div className="w-full h-24 flex items-center justify-between px-5">
                <h1 className="text-xl lg:text-4xl font-medium">Recent blogs</h1>
                <Link href={`/dashboard/createNewPost/${orginalData.length + 1}`} className="border rounded-lg border-sky-700 text-sm sm:text-base lg:text-lg text-sky-700 w-[45%] lg:w-[13%] h-14 gap-1 flex items-center justify-center">
                <Add size="32" className="text-sky-700"/> Add New
                </Link>
            </div>
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 px-3 md:px-0">
                {reverseddata.map((item,index : number)=>(
                    <div key={index} className="col-span-1 row-span-1 h-20 border shadow rounded-xl grid grid-cols-3 grid-rows-2 dark:border-zinc-900">
                        ...
                        <h1 className="pl-3 flex items-center capitalize col-start-2 col-end-4 row-span-1 text-sm md:text-xl font-medium text-ellipsis overflow-hidden whitespace-nowrap">{item.title}</h1>
                        <div className="px-3 flex items-center justify-between col-start-2 col-end-4 row-span-1 row-start-2">
                            ...
                            <Link href={`/dashboard/editPost/${item.id}`} className="text-xs md:text-base flex items-center justify-center w-1/3 h-[80%] gap-1 border rounded text-sky-900 border-sky-700"><Edit size="22" className="text-sky-900 hidden sm:block"/>Edit</Link>
